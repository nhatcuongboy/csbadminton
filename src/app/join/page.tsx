...
export default function JoinPage() {
  ...
  useEffect(() => {
    async function fetchSessions() {
      try {
        ...
        // Filter only PREPARING or IN_PROGRESS sessions
  ...
  const handleSessionChange = async (e: any) => {
    ...
    try {
      ...
      // Get the list of players who were created in the session
      const players = sessionData.players || [];

      // Filter to only get pre-filled players who haven't been confirmed yet
  ...
  const handlePlayerChange = (e: any) => {
    ...
    // Find the selected player's information
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!sessionId || !playerNumber || !selectedPlayer) {
      toast.error("Please select a session and player number");
      return;
    }

    setIsSubmitting(true);

    // Redirect to confirmation page with session ID, player number, and player ID
    try {
      window.location.href = `/join/confirm?sessionId=${sessionId}&playerNumber=${playerNumber}&playerId=${selectedPlayer.id}`;
    } catch (error) {
